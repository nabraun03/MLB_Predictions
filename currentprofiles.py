...
def add_stats_to_dict(prefix, stats, dict):
...
def generate_full_rosters(rosters, p):
    ...
    for game in rosters:
        ...
        add_stats_to_dict("away_SP", away_pitcher_stats, game_dict)
        add_stats_to_dict("home_SP", home_pitcher_stats, game_dict)
        ...
        for i in range(1, 10):
            ...
            add_stats_to_dict(f"home_batter_{i}", home_batter_stats, game_dict)
            add_stats_to_dict(f"away_batter_{i}", away_batter_stats, game_dict)
            add_stats_to_dict(
                f"home_fielder_{home_position}", home_fielder_stats, game_dict
            )
            add_stats_to_dict(
                f"away_fielder_{away_position}", away_fielder_stats, game_dict
            )
